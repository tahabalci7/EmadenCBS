...
| Metadata / KML naming | `project_info_extractor`, `kml_exporter`, `mine_type_allowlist` | Restore `{İL}/{Ek-1\|Ek-2}/{sicil} - {firma} - {maden_cinsi}.kml`. PR #2 (`33cc525`) inverted the stem to company-then-sicil and kept flat GUI writes. Ek-1 / Ek-2 stay in the folder. Missing il/Ek/sicil/firma/maden cinsi stay visible. Unreadable sicil is not invented: `license_no_missing` means Destekci must ask the user before overwrite export. `mine_type_allowlist` keeps only MAPEG-style mineral/commodity tokens after extraction (İ/I and diacritic fold; hyphen vs slash join preserved). Generic/operational leftovers (`MADEN`, `İŞLETME`, `RUHSAT`, `ATIK`, `AÇIK`, bare `CEVHER*`, `TAŞ`) are dropped. `TAŞ` is rejected — too often a quarry leftover, not a class. Zero valid tokens → `Bilinmiyor` + `mine_type_missing` (Destekci asks; do not invent). |
