...
| EK-1 selected-site coordinate appendix | `table_index`, `table_detector` | Find **PROJE İÇİN SEÇİLEN YERİN KOORDİNATLARI** (EK-1 / Ek 1 / SEÇİLEN YER / SEÇİLEN ALAN variants; Turkish İ/I fold + OCR noise) first, then export **every** coordinate table under that body heading until the next major unrelated section (EK-2, KAYNAKLAR). İÇİNDEKİLER dotted leaders are not the body start. TOC lines still add those pages to `target_pages` beyond max_pages=150. Full-PDF scan is fallback, not the primary path. Folder `EK-2` (PTD) is not this appendix label. |
