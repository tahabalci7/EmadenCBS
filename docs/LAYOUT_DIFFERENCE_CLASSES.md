...
| EK-1 selected-site coordinate appendix | `table_index` | İÇİNDEKİLER / EKLER lines such as `EK-1 PROJE İÇİN SEÇİLEN YERİN KOORDİNATLARI`, `Ek 1 …` (no dotted leader / page number), `Ek 1- …`, line-broken `EK-1` + `PROJE İÇİN SEÇİLEN YERİN KOORD…`, or `1- Proje için seçilen yerin koordinatları` add those body pages to `target_pages` even when they are **beyond max_pages=150**. Unnumbered TOC lines resolve via a late appendix body scan. Folder `EK-2` (PTD) is not this appendix label. |
| Scanned EK-1 coordinate appendix | `table_index`, `pdf_text_extraction_service` | Appendix title / “koordinatları Ek-1’de” is in the text layer, but UTM rings are image-only. Live export OCRs the planned appendix pages when those pages have no 6-digit UTM pairs. A dust-monitor / hava kalitesi X/Y table is not a substitute. |
